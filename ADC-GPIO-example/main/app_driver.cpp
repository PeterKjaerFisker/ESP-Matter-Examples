...
{
    return (u_int16_t)getAdc() * 100;
}

uint16_t app_driver_read_humidity(uint16_t endpoint_id)
{
    return (u_int16_t)getGpio() * 100;
}
...
static void DHT22_task(void *pvParameter)
{
    ...
    for (;;)
    {
        ...
        int ret = read_gpio_data();
        int adc_ret = read_adc_data();

        errorHandler(ret);
        errorHandler(adc_ret);
...
app_driver_handle_t app_driver_DHT_sensor_init()
{
    ...
    xTaskCreate(&DHT22_task, "DHT22_task", 5120, NULL, DHT22_TASK_PRIORITY, NULL);
